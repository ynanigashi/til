...
//     HH、MM、SSはそれぞれ時間、分、秒を2桁で表した文字列
  string to_str(){
    ostringstream sh, sm, ss;
    sh << setfill('0') << setw(2) << hour;
    sm << setfill('0') << setw(2) << minute;
...
//     diff_second の値は -86400 ~ 86400 の範囲を取とりうる
  void shift(int diff_second){
    int time_second = hour * 3600 + minute * 60 + second;
    if(time_second + diff_second < 0) {
      time_second = (86400 + (time_second + diff_second)) % 86400;
    } else {
      time_second = (time_second + diff_second) % 86400;
    }
